...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Assignment1_V2_M) == (NULL)) &&
    !rtmGetStopRequested(Assignment1_V2_M);
  while (runModel) {
    TgtConnPreStep(0);
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(Assignment1_V2_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(Assignment1_V2_M, true);
      }

      if (rtmGetStopRequested(Assignment1_V2_M) == true) {
    ...
    stopRequested = !((rtmGetErrorStatus(Assignment1_V2_M) == (NULL)) &&
                      !rtmGetStopRequested(Assignment1_V2_M));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Assignment1_V2_M, "stopping the model");
  runModel = 0;
}
...
void *backgroundTask(void *arg)
{
  while (runModel) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(Assignment1_V2_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(Assignment1_V2_M, true);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Assignment1_V2_M, 0);
  ...
  /* Initialize model */
  Assignment1_V2_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(Assignment1_V2_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(Assignment1_V2_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(Assignment1_V2_M, true);
    }
  }
